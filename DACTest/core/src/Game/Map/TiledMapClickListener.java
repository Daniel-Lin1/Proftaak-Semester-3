                    ...
                    if (buildings.get(i).getSelected() == true) {
                        ...
                        if (canSpawn == true)
                        {
                            ...
                            if (Gdx.input.isKeyPressed(Input.Keys.NUM_1)) {
                                ...
                                gameManager.getGmc().broadcastSpawnUnit("unit",UnitType.Knight ,tile.getCoordinate().getX(),tile.getCoordinate().getY());
                            } else if (Gdx.input.isKeyPressed(Input.Keys.NUM_2)) {
                                gameManager.getOwnPlayer().BuyUnit(uPB.produceUnit(UnitType.PikeMan));
                                gameManager.getGmc().broadcastSpawnUnit("unit",UnitType.PikeMan ,tile.getCoordinate().getX(),tile.getCoordinate().getY());
                            } else if (Gdx.input.isKeyPressed(Input.Keys.NUM_3)) {
                                gameManager.getOwnPlayer().BuyUnit(uPB.produceUnit(UnitType.Archer));
                                gameManager.getGmc().broadcastSpawnUnit("unit",UnitType.Archer ,tile.getCoordinate().getX(),tile.getCoordinate().getY());
                            }
                        }
                    }
                }
